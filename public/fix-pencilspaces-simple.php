<?php
// Simple PencilSpaces API Configuration Fix
...
try {
    ...
    if ($checkApiKey->fetch()) {
        ...
        // Insert new - using only the columns we know exist
        $insertApiKey = $pdo->prepare("INSERT INTO settings (key, \"group\", value, type, public, enumerable, sort, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
    ...
    if ($checkApiUrl->fetch()) {
        ...
        // Insert new
        $insertApiUrl = $pdo->prepare("INSERT INTO settings (key, \"group\", value, type, public, enumerable, sort, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
